...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	if (label == nil)
	{
        ...
        label.font = [UIFont fontWithName:@"GEDinarTwo-Medium" size:20];
        label.textColor = [UIColor darkGrayColor]; //HUD_STATUS_COLOR;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	...
	if (label.text != nil)
	{
		...
		labelRect = [label.text boundingRectWithSize:CGSizeMake(300, 300) options:options attributes:attributes context:NULL];

		labelRect.origin.x = 25;
		labelRect.origin.y = 66;
        labelRect.size.height = 30;
        
		hudWidth = labelRect.size.width + 60;
...
//-------------------------------------------------------------------------------------------------------------------------------------------------
{
	if (self.alpha == 1)
	{
		...
		[UIView animateWithDuration:0.20 delay:0 options:options animations:^{
